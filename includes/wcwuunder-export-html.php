...
if(empty($check_company)){ ?>
	<div class="warning">
		<img src="<?php echo Woocommerce_Wuunder::$plugin_url. 'assets/images/wuunder_logo.png' ?>" style="padding:20px 30px 20px 20px; width:180px;">
		<h2>Uw instellingen zijn niet volledig ingevuld</h2>
		<p>Controleer de gegevens van uw <b>"Standaard afhaaladres"</b>.</p>
		<a class="btn btn-primary" href="<?php echo admin_url('admin.php?page=wc-settings&tab=wuunder'); ?>">Controleer uw instellingen</a>
	</div>
<?php 
	exit;
}
...
<?php if($_GET['label'] == 'retour'){ ?>
	<h1><img style="width:34px;" src="<?php echo dirname(plugin_dir_url(__FILE__)) ?>/assets/images/create-icon.png"> Retourlabel aanmaken</h1>
<?php }else{ ?>
	<h1><img style="width:34px;" src="<?php echo dirname(plugin_dir_url(__FILE__)) ?>/assets/images/create-icon.png"> Verzendlabel aanmaken</h1>
<?php } ?>
	            ...
	            <div class="panel panel-default">
	                <div class="panel-heading">
	                	<div class="row">
			                ...
			                <div class="col-sm-8">
			                <?php if($_GET['label'] == 'retour'){ ?>
			                	<select class="pickup-address form-control" id="inputType" name="data[pickup_address]">
		            ...
		            <?php }else{ ?>
						...
						<?php } else { ?>
		                    ...
		                    <?php echo $row['straat'] ?> <?php echo $row['huisnummer'].$row['huisnummertoevoeging'] ?><br/>
		                    <?php echo $row['postcode'] ?> <?php echo $row['woonplaats'] ?> <?php echo $row['landcode']; ?><br/>
	    ...
	    <div class="panel panel-default">
	        ...
	        <div class="panel-body">
	            <div class="form-group has-success">
	                ...
	                <input type="text" class="form-control input-sm" name="data[phone_number]" id="wuunderPhonenumber" value="<?php echo $row['telefoon']; ?>" required="">
	            </div>
	        </div>
	    </div>
	    ...
	    <div class="panel panel-default">
	        ...
	        <div class="panel-body">
	                ...
	                <div class="row">
	                    <div class="col-xs-4">
	                        <div class="input-group">
	                            <input class="form-control input-sm" name="data[length]" id="wuunderLength" type="number" min="1" value="<?php echo $size[0] ?>" required="">
	                            <div class="input-group-addon input-sm">cm</div>
	                        </div>
	                    </div>
	                    <div class="col-xs-4">
	                        <div class="input-group">
	                            <input class="form-control input-sm" name="data[width]" id="wuunderWidth" type="number" min="1" value="<?php echo $size[1]; ?>" required="">
	                            <div class="input-group-addon input-sm">cm</div>
	                        </div>
	                    </div>
	                    <div class="col-xs-4">
	                        <div class="input-group">
	                            <input class="form-control input-sm" name="data[height]" id="wuunderHeight" type="number" min="1" value="<?php echo str_replace(' cm', '', $size[2]); ?>" required="">
	                            <div class="input-group-addon input-sm">cm</div>
	                        </div>
	                    </div>
	                </div>
	            ...
	            <div class="form-group">
	                ...
	                <div class="input-group">
	                    <input class="form-control input-sm" name="data[weight]" id="wuunderWeight" type="number" value="<?php echo $total_weight; ?>" required="">
	            ...
	            <div class="form-group">
	                ...
	                <input class="form-control input-sm" name="data[description]" id="wuunderReference" type="text" placeholder="Wat is de inhoud van de zending die je gaat versturen?" value="<?php foreach ($row['bestelling'] as $product) { echo $product['name'].$product['variation']; if (next($row['bestelling'])) { echo ', '; }} ?>" required="">
	            </div>

	            <div class="form-group">
	                <label class="control-label" for="wuunderMessage">Chat bericht</label>
	                <textarea class="form-control input-sm" name="data[personal_message]" id="wuunderMessage" placeholder="Stuur een persoonlijk bericht naar de ontvanger"></textarea>
	            </div>

	            <div class="form-group">
	                <?php if($_GET['label'] == 'retour'){ ?>
						<input type="hidden" name="action" value="wcwuunder-retour">
						<input type="submit" value="Vraag retourlabel aan" class="button-wuunder btn save_order btn-primary tips">
					<?php }else{ ?>
						<input type="hidden" name="action" value="wcwuunder-export">
						<input type="submit" value="Vraag verzendlabel aan" class="button-wuunder btn save_order btn-primary tips">
					<?php } ?>
	            </div>
	        </div>
	    </div>
